...
## Security & RAG Integration Notes

- **Network Exposure**: Always bind inference servers (`sglang`, `vLLM`) to `127.0.0.1` locally rather than `0.0.0.0` unless protected by authenticated reverse proxies or firewalls.
- **RAG Indirect Prompt Injection**: When feeding OCR-parsed document text into a RAG pipeline, sanitize extracted text and ensure downstream LLMs treat retrieved documents strictly as untrusted context.
- **Temp Storage**: Automatically clean up temporary image directories generated during PDF-to-image conversion to prevent sensitive file residue on disk.
